...
class SammedPredictor:
    ...
    def set_image(self,image: np.ndarray, image_format: str = "RGB") -> None:
        ...
        #------------------------------------------
        # 双线性插值
        resized_image = cv2.resize(input_image, (self.model.image_encoder.img_size, self.model.image_encoder.img_size), interpolation=cv2.INTER_NEAREST)
        resized_image = resized_image[None, :, :, :]
        resized_image = np.transpose(resized_image,(0,3,1,2))
        input_image = torch.tensor(resized_image)
        ...
        # print('input_image.shap', input_image.shape)
        #--------------------------------------------------------------------------------------
