...
自由形状标注（Polygon ROI）
交互：单击添加点，双击结束当前标注。
多边形：Catmull-Rom 闭合样条插值，统计面积/HU。
"""
import math
import numpy as np
import vtk
...
def _spline_with_closing_line(ctrl_pts, n_seg=30):
    """
    样条曲线 + 直线闭合：
    - 相邻控制点之间用 Catmull-Rom 样条
    - 最后一个点到第一个点用直线连接
    """
    if len(ctrl_pts) < 2:
        return list(ctrl_pts)
    # 开放样条（不循环）
    spts = _spline_points(ctrl_pts, closed=False, n_seg=n_seg)
    # 加直线回到起始点
    spts.append(ctrl_pts[0])
    return spts
...
class PolygonAnnotation:
    """自由形状标注：Catmull-Rom 闭合样条 + HU/面积统计"""
    ...
    def _build(self):
        spts = _spline_with_closing_line(self.ctrl_pts)
        self._curve_actor, self._curve_pts, self._curve_poly = \
            _make_polyline_actor(spts, _COLOR_POLYGON, lw=1.8)
    ...
    def _update_stats(self):
        try:
            ...
            cur     = self._viewer.GetSlice()
            ax0, ax1 = _plane_axes(self.vt)

            spts = _spline_with_closing_line(self.ctrl_pts)
            if not spts:
                self._text_actor.SetInput("No data"); return

            u_vals = [p[ax0] for p in spts]
            v_vals = [p[ax1] for p in spts]
            u_min, u_max = min(u_vals), max(u_vals)
            v_min, v_max = min(v_vals), max(v_vals)

            def w2p(w, ax): return int(round((w - origin[ax]) / spacing[ax]))
            u0 = max(0, w2p(u_min, ax0)); u1 = min(dims[ax0]-1, w2p(u_max, ax0))
            v0 = max(0, w2p(v_min, ax1)); v1 = min(dims[ax1]-1, w2p(v_max, ax1))

            poly2d = [(p[ax0], p[ax1]) for p in spts]

            def point_in_poly(px, py):
                n = len(poly2d); inside = False
                j = n - 1
                for i in range(n):
                    xi, yi = poly2d[i]; xj, yj = poly2d[j]
            ...
            for ui in range(u0, u1+1):
                for vi in range(v0, v1+1):
                    wu = origin[ax0] + ui * spacing[ax0]
                    wv = origin[ax1] + vi * spacing[ax1]
                    if point_in_poly(wu, wv):
                        if self.vt == "XY":
                            scalars.append(img.GetScalarComponentAsFloat(ui, vi, max(0,min(cur,dims[2]-1)), 0))
                        elif self.vt == "YZ":
                            scalars.append(img.GetScalarComponentAsFloat(max(0,min(cur,dims[0]-1)), ui, vi, 0))
                        else:
                            scalars.append(img.GetScalarComponentAsFloat(ui, max(0,min(cur,dims[1]-1)), vi, 0))

            area = 0.0
            n = len(poly2d)
            for i in range(n):
                j = (i+1) % n
                area += poly2d[i][0] * poly2d[j][1]
                area -= poly2d[j][0] * poly2d[i][1]
            area = abs(area) / 2.0 / 100.0  # mm² → cm²
            ...
            if scalars:
                ...
                self._text_actor.SetInput(f"Area:{area:.3f} cm\u00b2")

            if self.ctrl_pts:
                p = self.ctrl_pts[0]
                try:
                    sp = self._viewer.GetInput().GetSpacing()
                    self._text_actor.GetPositionCoordinate().SetValue(
                        u_max + sp[ax0]*3, v_max + sp[ax1]*3, self._depth_val or p[2])
                except Exception:
                    self._text_actor.GetPositionCoordinate().SetValue(u_max+3, v_max+3, p[2])
        except Exception:
            logger.debug("polygon stats error", exc_info=True)
...
class _PolygonHandler:
    """Polygon 交互 handler。单击添加点，双击结束标注。"""
    ...
    def on_key(self, caller, ev):
        ...
        if key in ("Delete", "BackSpace") and self._selected_ann:
            ...
            if self._moving_handle_ann is not None:
                ...
                self._drawing = False; self._ctrl_pts = []
                self._clear_preview(); self._render()
    ...
    def _update_preview(self, cursor=None):
        ...
        # 样条曲线 + 直线闭合到起始点
        spts = _spline_with_closing_line(pts)
    ...
    def activate(self):
        ...
        for ortho, vt in configs:
            if self._handlers[vt] is None:
                h = _PolygonHandler(ortho, vt, self._annotations[vt])
                self._handlers[vt] = h
...
class PolygonROIManager(_BaseROIManager):
    def __init__(self, view_model):
